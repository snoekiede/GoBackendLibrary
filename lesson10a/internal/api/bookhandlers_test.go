...
type MockQueries struct {
	...
	DeleteBookFunc             func(ctx context.Context, id int32) (int32, error)
...
func (m *MockQueries) DeleteBook(ctx context.Context, id int32) (int32, error) {
...
func TestCreateBook(t *testing.T) {
	tests := []struct {
		...
		{
			name: "invalid request body: no title",
			requestBody: CreateBookRequest{
				Title:  "",
				Author: "Author",
			},
		...
		{
			name: "invalid request body: no author",
			requestBody: CreateBookRequest{
				Title:  "Title",
				Author: "",
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			store := NewBookHandler(mockDB, nil)
...
func TestGetUserBorrowedBooks(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			store := NewBookHandler(mockDB, nil)
			req := httptest.NewRequest(http.MethodGet, "/books/user/"+tt.userID+"/borrowed", nil)
			rr := httptest.NewRecorder()
			...
			if tt.expectedStatus == http.StatusOK {
				var books []models.BorrowedBooksRowResponse
				if err := json.Unmarshal(rr.Body.Bytes(), &books); err != nil {
					t.Fatalf("failed to unmarshal response: %v", err)
				}
...
func TestGetOverdueBooks(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedStatus == http.StatusOK {
				var books []models.OverdueBooksRowResponse
				if err := json.Unmarshal(rr.Body.Bytes(), &books); err != nil {
					t.Fatalf("failed to unmarshal response: %v", err)
				}
